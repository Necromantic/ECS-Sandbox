...
namespace toinfiniityandbeyond.Rendering2D
{
    ...
    public class SpriteInstanceRendererSystem : ComponentSystem
	{
	    ...
	    protected override void OnCreateManager(int capacity)
	    {
	        ...
	        instanceRendererGroup = GetComponentGroup(typeof(SpriteInstanceRenderer), typeof(TransformMatrix), typeof(SpriteColor), typeof(SpriteUniformScale));
	    }
	    ...
	    protected override void OnUpdate()
		{
            ...
            for (int i = 0;i != cacheduniqueRendererTypes.Count;i++)
            {
                ...
                var colors = instanceRendererGroup.GetComponentDataArray<SpriteColor>();
                var scales = instanceRendererGroup.GetComponentDataArray<SpriteUniformScale>();
                ...
                {
                    material = new Material(Shader.Find("Custom/Instanced/Transparent/Diffuse"))
                    {
                ...
                while (beginIndex < transforms.Length)
                {
                    ...
                    // Scale matrices and prepare the color array for batch rendering
                    int k = 0;
                    for (int j = beginIndex; j < beginIndex + length; ++j)
                    {
                        matricesArray[k] = matricesArray[k] * Matrix4x4.Scale(new Vector3(scales[k].Value, scales[k].Value, scales[k].Value));
